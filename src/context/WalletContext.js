...
const processRefundLogs = logs => {
  if (!logs) return [];
  return logs
    // Add sorting by creationDate (descending) before mapping
    .sort((a, b) => {
      const dateA = new Date(a.creationDate || 0); // Use 0 for invalid dates to push them to the beginning
      const dateB = new Date(b.creationDate || 0); // Use 0 for invalid dates
...
const processWalletBills = bills => {
  ...
  return bills.map(bill => {
    ...
    if (description.toLowerCase() === 'string') {
      ...
      description.toLowerCase().includes('design fee')
    ) {
      ...
      if (percentage && orderId) {
        ...
        description = `Thanh toán phí thiết kế cho đơn hàng ${
          bill.orderId || ''
        }`;
      }
...
//console.log("user", user);
  ...
  const fetchWalletData = async (forceRefresh = false) => {
    try {
      ...
      if (!walletId) {
        console.warn(
          'Không tìm thấy ID ví trong dữ liệu người dùng đã lưu. Việc tải có thể thất bại.',
        );
      ...
      const formattedPurchases = processWalletBills(
        userWalletResponse.bills || [],
      );
  ...
  const createVnpayPayment = async amount => {
    ...
    try {
      console.log(
        `Tạo giao dịch VNPay với số tiền: ${numericAmount} cho thiết bị di động`,
      );
